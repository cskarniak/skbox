...
- **Veille** : après `IDLE_S` secondes sans toucher, le Wi-Fi est coupé et l'appareil passe en *light sleep* ; l'en-tête affiche « en veille » et le bouton « Actualiser » devient **« Activer »** (noir). Pendant la veille, seul « Activer » réveille l'écran (et recharge les données si elles ont plus d'une minute) ; un toucher ailleurs est ignoré et l'appareil se rendort aussitôt. Il se réveille aussi seul pour se rafraîchir, puis se rendort : toutes les `REFRESH_S` secondes le jour, toutes les `NIGHT_REFRESH_S` secondes la nuit (de `NIGHT_START_H` à `NIGHT_END_H`, heure locale fournie par skbox).
- **Économie d'énergie** : un rafraîchissement automatique ne redessine tout l'écran que pour un vrai changement (état d'une prise, de la chaudière, d'un capteur, date, ou écart d'au moins 0,3 °C / 3 % d'humidité par rapport à l'affiché), et en mode rapide ; sinon seule la ligne d'état est mise à jour. Un redessin complet en haute qualité nettoie la rémanence une fois par heure. Le Wi-Fi est coupé `WIFI_IDLE_OFF_MS` (5 s) après le dernier échange, même écran éveillé.
- **Intervalle imposé par skbox** : le réglage `displayRefreshSeconds` (`PUT /api/settings/displayRefreshSeconds` avec `{"value": "1800"}`, ≥ 60 s ; supprimer ou vider pour revenir au défaut) remplace `REFRESH_S` le jour, sans reflasher.
...
- **Diagnostic à distance** : chaque requête `GET /api/display/summary` porte, en plus de `bat`/`mv`/`chg`, `rst` (raison du dernier démarrage, `esp_reset_reason()` : 1 = mise sous tension, 3 = logiciel, 4 = plantage, 5/6/7 = chien de garde, 9 = baisse de tension), `last` (dernière étape notée en mémoire non volatile avant ce démarrage : 1 = éveillé, 2 = en veille, 3 = réveil par minuterie, 4 = réveil par toucher, 5 = attente du relâchement du tactile avant la veille), `rr` (code matériel brut du reset, ex. 1 = mise sous tension, 21 = reset par l'USB), `boot` (compteur de démarrages) et `wk` (dernier réveil : `t` minuterie, `p` toucher), et pour la consommation : `aw` (durée d'éveil du cycle précédent, ms), `wc` (durée de la dernière connexion Wi-Fi, ms), `fc` (1 = la reconnexion rapide a abouti), `rd` (dernier redessin automatique : `q` nettoyage haute qualité, `f` écran complet rapide, `s` ligne d'état seule), puis, cumulés depuis la requête précédente : `nt`/`ng`/`no` (réveils par minuterie / tactile / autre ou veille refusée), `at` (temps éveillé, ms), `sl` (temps endormi, ms). Lecture : `grep ESP32HTTPClient /var/log/nginx/access.log` sur skbox-mini.
