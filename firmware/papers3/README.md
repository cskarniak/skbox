...
- **Économie d'énergie** : un rafraîchissement automatique ne redessine tout l'écran que si une valeur affichée a changé (sinon seule la ligne d'état, en mode rapide), avec un redessin complet de nettoyage au moins une fois par heure. Le Wi-Fi est coupé `WIFI_IDLE_OFF_MS` (5 s) après le dernier échange, même écran éveillé ; il se reconnecte en ~1 s au besoin.
