...
export function OverlayChart({ series, height = 280 }: { series: OverlaySeries[]; height?: number }) {
  ...
  const axisValues = (axisId: 'left' | 'right') =>
    series.filter((s) => yAxisIdFor(getValueMeta(s.valueKey).unit) === axisId).flatMap((s) => s.data.map((p) => p.value));
  ...
  const leftAxis = useMemo(() => buildAxis(axisValues('left')), [series, secondaryUnit]);
  const rightAxis = useMemo(() => buildAxis(axisValues('right')), [series, secondaryUnit]);
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <LineChart>
        ...
        {series.map((s) => {
          const isBinary = s.data.length > 0 && s.data.every((p) => p.value === 0 || p.value === 1);
          return (
            <Line
              key={s.id}
              data={s.data}
