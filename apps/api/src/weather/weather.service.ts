...
export type MapVariable = 'pressure' | 'temperature';
...
export class WeatherService {
  ...
  // Carte "masses d'air" maison : Open-Meteo accepte une liste de lieux en un seul appel
  // (latitude/longitude en listes parallèles séparées par des virgules), ce qui permet de
  // récupérer une grille de points en une requête plutôt qu'un appel par point. On en tire
  // une carte de pression (proxy des masses d'air : dépressions/anticyclones) ou de
  // température, interpolée côté client.
  async getMap(variable: MapVariable): Promise<WeatherMap> {
    const field = variable === 'pressure' ? 'pressure_msl' : 'temperature_2m';
